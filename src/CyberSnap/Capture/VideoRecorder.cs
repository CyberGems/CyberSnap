...
public sealed class VideoRecorder : IDisposable
{
    ...
    private static string? ResolveFfmpegPath()
    {
        ...
        string[] candidates =
        {
            ...
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "CyberSnap", "ffmpeg.exe"),
        };
    ...
    public string StopAndEncode(string outputPath)
    {
        ...
        if (!File.Exists(outputPath) || new FileInfo(outputPath).Length == 0)
            throw new InvalidOperationException($"Video encoding failed â€” no output file produced. {_ffmpegStderr}");
    ...
    private bool MuxAudio(string videoPath)
    {
        ...
        try
        {
            ...
            {
                // Mux failed â€” keep the original video without audio
