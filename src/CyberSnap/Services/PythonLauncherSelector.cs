...
internal static partial class PythonLauncherSelector
{
    ...
    public static string BuildOnnxRuntimeMissingVersionMessage(IEnumerable<LauncherEntry> entries)
    {
        ...
        return discovered.Count == 0
            ? "CyberSnap needs Python 3.11, 3.12, 3.13, or 3.14 installed to set up the local sticker and upscale runtimes."
            : $"CyberSnap needs Python 3.11, 3.12, 3.13, or 3.14 to set up the local sticker and upscale runtimes. Found: {string.Join(", ", discovered)}.";
    }
