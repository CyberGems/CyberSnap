...
public static class InstallService
{
    public static string DefaultInstallPath =>
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Programs", "CyberSnap");
    ...
    public static string? GetInstalledLocation()
    {
        try
        {
            using var key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\CyberSnap");
    ...
    /// <summary>Kill any running CyberSnap processes (other than this one).</summary>
    public static void KillRunningInstances()
    {
        var currentPid = Environment.ProcessId;
        foreach (var proc in Process.GetProcessesByName("CyberSnap"))
        {
            ...
            catch (Exception ex)
            {
                AppDiagnostics.LogWarning(
                    ...
                    $"Failed to close running CyberSnap process {processId}: {ex.Message}",
                    ex);
            }
        }
    }

    /// <summary>Install CyberSnap to the target directory.</summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        onProgress?.Invoke("Closing any running CyberSnap instances...");
        ...
        if (string.IsNullOrWhiteSpace(sourceDir) || !Directory.Exists(sourceDir))
            throw new InvalidOperationException("Unable to locate the running CyberSnap application folder.");
        var currentExe = Environment.ProcessPath;
        if (string.IsNullOrWhiteSpace(currentExe) || !File.Exists(currentExe))
            throw new InvalidOperationException("Unable to locate the running CyberSnap executable.");

        cancellationToken.ThrowIfCancellationRequested();
        var targetExe = Path.Combine(targetDirNorm, "CyberSnap.exe");
            ...
            try
            {
                ...
                CreateShortcut(
                    Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                        "Microsoft", "Windows", "Start Menu", "Programs", "CyberSnap.lnk"),
                    targetExe);
            }
            catch (Exception ex)
            {
        ...
        if (desktopShortcut)
        {
            try
            {
                ...
                CreateShortcut(
                    Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "CyberSnap.lnk"),
                    targetExe);
            }
            catch (Exception ex)
            {
    ...
    {
        targetDir = NormalizeTargetDirectory(targetDir);
        var targetExe = Path.Combine(targetDir, "CyberSnap.exe");
        var args = showOnboarding ? "--post-install" : "";
        if (!TryLaunch(targetExe, targetDir, args))
            throw new InvalidOperationException("CyberSnap was installed, but the installed copy could not be launched.");
    }
    ...
    public static void ApplyUpdateFromZip(string packagePath, string targetDir, string? versionLabel = null, bool launchAfter = true, Action<string>? onProgress = null)
    {
        ...
        var targetDirNorm = targetDir.TrimEnd('\\', '/');
        var sourceRoot = Path.Combine(Path.GetTempPath(), "CyberSnap", "ApplyUpdate", Guid.NewGuid().ToString("N"));

        try
        {
            Directory.CreateDirectory(sourceRoot);
            onProgress?.Invoke("Waiting for CyberSnap to close...");
            ...
            if (launchAfter)
            {
                onProgress?.Invoke("Launching updated CyberSnap...");
                if (!TryLaunch(targetExe, targetDirNorm, ""))
                    throw new InvalidOperationException("The update was applied, but CyberSnap could not be restarted.");
            }
        }
        finally
        {
    ...
    private static void CopyInstallPayload(string sourceDir, string currentExe, string targetDir, CancellationToken cancellationToken)
    {
        ...
        CopyFileWithRetry(currentExe, Path.Combine(targetDir, "CyberSnap.exe"), cancellationToken);
    ...
    {
        var directExe = Path.Combine(extractedRoot, "CyberSnap.exe");
        if (File.Exists(directExe))
            return extractedRoot;

        var childDirs = Directory.EnumerateDirectories(extractedRoot).ToList();
        if (childDirs.Count == 1 && File.Exists(Path.Combine(childDirs[0], "CyberSnap.exe")))
            return childDirs[0];

        var exe = Directory.EnumerateFiles(extractedRoot, "CyberSnap.exe", SearchOption.AllDirectories).FirstOrDefault();
        if (!string.IsNullOrWhiteSpace(exe))
    ...
    private static string GetRequiredParentDirectory(string path, string context)
    {
        if (string.IsNullOrWhiteSpace(path))
            throw new InvalidOperationException($"CyberSnap couldn't resolve the {context}. Try installing to the default path.");

        var directory = Path.GetDirectoryName(path);
        if (string.IsNullOrWhiteSpace(directory))
            throw new InvalidOperationException($"CyberSnap couldn't resolve the parent folder for '{path}'. Try installing to the default path.");
    ...
    private static void CreateShortcut(string shortcutPath, string targetExe)
    {
        ...
        try
        {
            ...
            sc.Description = "CyberSnap screenshot tool";
            sc.Save();
        }
    ...
    internal static bool LooksLikeBuildOutputPath(string path)
    {
        ...
        return normalized.Contains(@"\bin\Debug\", StringComparison.OrdinalIgnoreCase)
            ...
            || normalized.Contains(@"\src\CyberSnap\bin\", StringComparison.OrdinalIgnoreCase);
    }
}
