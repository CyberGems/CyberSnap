...
public partial class ToastWindow
{
    ...
    /// <summary>
    /// Preloader toast for cold-opening heavy windows (Editor / Gallery / Configuration).
    /// Always marshals to the WPF UI thread <b>synchronously</b> and forces a render pass so
    /// the toast is visible before the caller blocks on window construction.
    /// </summary>
    public static void ShowStartingStatus(string title, string? body = null)
    {
        var spec = new ToastSpec
        {
            Title = title,
            Body = body ?? Services.LocalizationService.Translate("Preparing the workspace…"),
            IsSystemMessage = true,
            SuppressSound = true,
            DurationSeconds = 8,
        };
        ...
        // Sync Invoke: BeginInvoke would return before the toast paints, and the caller
        // would immediately block the UI thread constructing the target window.
        d.Invoke(() =>
        {
            Show(spec);
            FlushPresentation();
    ...
    /// <summary>
    /// Processes pending WPF layout/render so a toast shown just now is on screen before
    /// long-running work (e.g. WinForms EditorForm construction) freezes the dispatcher.
    /// </summary>
    public static void FlushPresentation()
    {
        var d = System.Windows.Application.Current?.Dispatcher;
        ...
        try
        {
            if (_current is { IsVisible: true })
                _current.UpdateLayout();

            // Nested dispatcher frames at render priorities — presents the toast frame.
            d.Invoke(new Action(() => { }), System.Windows.Threading.DispatcherPriority.Loaded);
            d.Invoke(new Action(() => { }), System.Windows.Threading.DispatcherPriority.Render);
            d.Invoke(new Action(() => { }), System.Windows.Threading.DispatcherPriority.ApplicationIdle);
        }
        catch
        {
            // Never let presentation flush break open flows.
        }
    }
