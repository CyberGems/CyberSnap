...
public partial class SettingsWindow
{
    ...
    private static string GetUploadProviderLogoPath(string? providerName)
    {
        if (string.IsNullOrWhiteSpace(providerName)) return "";
        var normalized = providerName.Trim().ToLowerInvariant().Replace(" ", "").Replace(".", "");
        return $"Assets/{normalized}_sq.png";
    }

    private static FrameworkElement? CreateProviderBadge(string? providerOrPath, bool isPath = false)
    {
        string logoPath = isPath ? (providerOrPath ?? string.Empty) : GetUploadProviderLogoPath(providerOrPath);
