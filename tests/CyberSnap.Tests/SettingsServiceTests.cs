...
public sealed class SettingsServiceTests
{
    ...
    public void DiagnosticsRedaction_HidesCommonSecretFormats()
    {
        var redacted = AppDiagnostics.RedactSensitiveText(
            "api_key=abc123 access_token=verysecret X-Api-Key: headersecret Bearer jwt.payload \"Token\": \"secret_value\"");
    ...
    {
        var source = File.ReadAllText(FindRepoFile("src/CyberSnap/Services/SettingsService.cs"));
    ...
    [Fact]
    public void PortableStorage_UsesCyberSnapSubfolderBesideApp()
    {
        var appDir = Path.Combine(Path.GetTempPath(), "CyberSnap-portable", Guid.NewGuid().ToString("N"));

        var storageDir = AppStoragePaths.ResolveStorageDirectory(appDir, isInstalled: false);

        Assert.Equal(Path.Combine(appDir, "CyberSnap"), storageDir);
        Assert.Equal(Path.Combine(appDir, "CyberSnap", "settings.json"), Path.Combine(storageDir, "settings.json"));
        Assert.Equal(Path.Combine(appDir, "CyberSnap", "logs"), Path.Combine(storageDir, "logs"));
    }

    [Fact]
    public void InstalledStorage_UsesRoamingFolder()
    {
        var appDir = Path.Combine(Path.GetTempPath(), "CyberSnap-installed", Guid.NewGuid().ToString("N"));
        ...
        Assert.EndsWith(Path.Combine("CyberSnap"), storageDir);
    }
