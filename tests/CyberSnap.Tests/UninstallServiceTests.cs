...
public sealed class UninstallServiceTests
{
    ...
    {
        var source = File.ReadAllText(RepoPath("src", "CyberSnap", "Services", "UninstallService.cs"));
    ...
    public void UninstallRegistryCleanupReportsUnavailableParentKeys()
    {
        ...
        Assert.Contains("key.DeleteSubKeyTree(\"CyberSnap\", throwOnMissingSubKey: false);", installedEntryBlock);
        ...
        Assert.Contains("key.DeleteValue(\"CyberSnap\", throwOnMissingValue: false);", startupEntryBlock);
    ...
    {
        var source = File.ReadAllText(RepoPath("src", "CyberSnap", "Services", "UninstallService.cs"));
        var lifecycleSource = File.ReadAllText(RepoPath("src", "CyberSnap", "App", "App.Lifecycle.cs"));
        ...
        Assert.Contains("throw new InvalidOperationException(\"CyberSnap could not resolve its executable path for startup.\");", setStartupBlock);
        Assert.Contains("existingKey.DeleteValue(\"CyberSnap\", throwOnMissingValue: false);", setStartupBlock);
        Assert.DoesNotContain("existingKey?.DeleteValue", setStartupBlock);
        Assert.Contains("key.SetValue(\"CyberSnap\", $\"\\\"{exe}\\\"\", RegistryValueKind.String);", setStartupBlock);
