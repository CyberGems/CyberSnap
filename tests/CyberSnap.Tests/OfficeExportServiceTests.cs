...
public sealed class OfficeExportServiceTests
{
    ...
    {
        var source = File.ReadAllText(RepoPath("src", "CyberSnap", "Services", "OfficeExportService.cs"));
    ...
    {
        var appPath = Path.Combine(Path.GetTempPath(), $"CyberSnap-test-app-{Guid.NewGuid():N}.exe");
