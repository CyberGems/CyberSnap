...
public class OcrResultWindowLifecycleTests
{
    ...
    {
        var source = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "OcrResultWindow.xaml.cs"));
        ...
        Assert.Contains("CyberSnap could not copy the OCR text. Keep the result window open and try again.", copyBlock);
        ...
        Assert.Contains("CyberSnap could not copy the translated text. Keep the result window open and try again.", translationBlock);
    ...
    {
        var xaml = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "OcrResultWindow.xaml"));
    ...
    public void TranslationControlsAndFooterActionsWrapWithSpacing()
    {
        ...
        Assert.Contains("Text=\"â†’\" FontSize=\"14\" Margin=\"0,0,6,6\"", xaml);
