...
public sealed class OverlayAccessibilityTests
{
    ...
    {
        var xaml = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "PreviewWindow.xaml"));
    ...
    {
        var xaml = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "PreviewWindow.xaml"));
        var previewCode = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "PreviewWindow.xaml.cs"));
        var previewActionsCode = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "PreviewWindow.Actions.cs"));
    ...
    {
        var previewCode = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "PreviewWindow.Actions.cs"));
    ...
    public void PreviewGifClipboardFailuresAreVisible()
    {
        ...
        Assert.Contains("CyberSnap could not copy the GIF file. The preview will still open; save or drag the GIF manually.", helperBlock);
    ...
    {
        var previewCode = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "PreviewWindow.Actions.cs"));
        var previewStateCode = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "PreviewWindow.xaml.cs"));
        ...
        Assert.Contains("CyberSnap could not open the saved file location. The file is still saved; open it from History or try again.", previewCode);
        Assert.Contains("CyberSnap could not open the preview target. Capture again or check History for another copy.", previewCode);
    ...
    public void PreviewDragMissingFileShowsVisibleError()
    {
        ...
        Assert.Contains("CyberSnap could not start the drag. The file is still saved; open it from History or try again.", previewCode);
        Assert.Contains("CyberSnap could not start the drag. The preview is still open; use Save or try the capture again.", previewCode);
    ...
    {
        var toastCode = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "ToastWindow.xaml.cs"));
        ...
        Assert.Contains("BuildToastActionFailureBody(\"CyberSnap could not delete the saved file. Open it from History or delete it manually in File Explorer.\", ex.Message)", deleteBlock);
    ...
    {
        var toastCode = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "ToastWindow.xaml.cs"));
        var toastStaticCode = File.ReadAllText(RepoPath("src", "CyberSnap", "UI", "ToastWindow.Static.cs"));
        ...
        Assert.Contains("CyberSnap could not open the saved file location. Try again from the toast, or open the folder manually.", openLocationBlock);
        ...
        var fileLocationErrorIndex = openLocationBlock.IndexOf("CyberSnap could not open the saved file location.", fileLocationLogIndex, StringComparison.Ordinal);
    ...
    public void ToastOfficeFeedbackDoesNotAttachMissingSavedFile()
    {
        ...
        Assert.Contains("BuildToastActionFailureBody(\"CyberSnap could not open the image with another app. Save the capture or open it from History, then try Windows Open with.\", ex.Message)", openWithBlock);
        ...
        Assert.Contains("BuildToastActionFailureBody(\"CyberSnap could not send the image to Office. Save the capture and insert it manually, or try another Office target.\", ex.Message)", officeBlock);
