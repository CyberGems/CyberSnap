...
public sealed class InstallServiceUpdateTests
{
    ...
    {
        var targetDir = Path.Combine(Path.GetTempPath(), "CyberSnap-tests", Guid.NewGuid().ToString("N"), "target");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "CyberSnap-tests", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(packageDir, "CyberSnap.exe"), "new executable");
            ...
            File.WriteAllText(Path.Combine(targetDir, "CyberSnap.exe"), "old executable");

            ZipFile.CreateFromDirectory(packageDir, zipPath);

            InstallService.ApplyUpdateFromZip(zipPath, targetDir, launchAfter: false);

            Assert.Equal("new executable", File.ReadAllText(Path.Combine(targetDir, "CyberSnap.exe")));
    ...
    public void InstallerOnlyFolder_DoesNotLookLikeFullPayloadTree()
    {
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(root, "CyberSnap.exe"), "installer");
    ...
    [Theory]
    [InlineData("C:\\Installed\\CyberSnap", "C:\\Portable\\CyberSnap", true, "C:\\Installed\\CyberSnap")]
    [InlineData("C:\\Installed\\CyberSnap", "C:\\Portable\\CyberSnap", false, "C:\\Portable\\CyberSnap")]
    [InlineData(null, "C:\\Portable\\CyberSnap", false, "C:\\Portable\\CyberSnap")]
    ...
    {
        var source = File.ReadAllText(RepoPath("src", "CyberSnap", "Services", "InstallService.cs"));
    ...
    public void InstallerShortcutAndRegistrationFailuresAreDiagnosable()
    {
        ...
        Assert.Contains("Failed to close running CyberSnap process", closeRunningBlock);
    ...
    public void LaunchInstalledReportsLaunchFailure()
    {
        ...
        Assert.Contains("CyberSnap was installed, but the installed copy could not be launched.", launchInstalled);
